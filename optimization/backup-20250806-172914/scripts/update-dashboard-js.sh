...
# Purpose: 現在のコンテキストサイズをダッシュボード用JSファイルに出力
...
if [ -f "$CONTEXT_DIR/.total_tokens_used" ]; then
    SESSION_TOKENS_USED=$(cat "$CONTEXT_DIR/.total_tokens_used")
else
    # ターン数 * 平均トークン数で推定
    if [ -f "$TURN_FILE" ]; then
        TURNS=$(cat "$TURN_FILE")
        SESSION_TOKENS_USED=$((TURNS * 5000))  # 1ターンあたり約5000トークンと推定
        echo "$SESSION_TOKENS_USED" > "$CONTEXT_DIR/.total_tokens_used"
    fi
fi
...
if [ -f "$FILE_LOG" ]; then
    ...
    BEGIN { print "[" }
    {
        ...
        # プロジェクトルートからの相対パス
        sub(/^\/Users\/[^\/]+\/dev3\/article-agent\//, "", $2)
...
window.dashboardData = {
  ...
  "session": {
    ...
    "totalTokensUsed": $SESSION_TOKENS_USED,
...
if [ "$STATUS_OVERALL" = "danger" ] || [ "$STATUS_OVERALL" = "critical" ]; then
    echo "" >&2
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" >&2
    echo "$STATUS_LEVEL $STATUS_MESSAGE - コンテキストサイズ: $(printf "%'d" $TOTAL_TOKENS) tokens" >&2
